...
create table if not exists public.notifications (
    id bigint generated by default as identity primary key,
    report_id bigint,
    recipient_user_id bigint references public.user_profiles(id) on delete cascade,
...
alter table public.user_profiles add column if not exists profile_image_path text;
...
alter table public.report_items drop constraint if exists report_items_user_id_fkey;
alter table public.report_items
...
alter table public.notifications
    ...
    foreign key (recipient_user_id) references public.user_profiles(id)
    on delete cascade;
...
begin
    if new.id is null then
        raise exception 'Profile ID cannot be null. All profiles must have a unique ID.';
    end if;
    return new;
end;
$$ language plpgsql;
...
-- Validate profile ID is always non-null for consistency
drop trigger if exists trg_user_profiles_validate_id on public.user_profiles;
create trigger trg_user_profiles_validate_id
